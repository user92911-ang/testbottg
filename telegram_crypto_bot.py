...
class CryptoBalanceBot:
    def __init__(self, telegram_token: str, coingecko_api_key: str = None):
        ...
        self.coingecko_api_key = coingecko_api_key # Store the API key
    ...
    async def get_eth_price(self, session: aiohttp.ClientSession) -> float:
        ...
        if self.coingecko_api_key:
            # Use the Pro (authenticated) endpoint
    ...
    def parse_addresses(self, text: str) -> List[str]:
        ...
        unique_addresses = list(dict.fromkeys(addr.lower() for addr in addresses))
        return [addr for addr in addresses if addr.lower() in unique_addresses]
    ...
    async def balance_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if len(addresses) > 200:
            await update.message.reply_text(f"❌ Too many addresses ({len(addresses)}). Please limit to 200 per request.")
            return
...
def main():
    ...
    # Read the new optional environment variable for CoinGecko
    COINGECKO_API_KEY = os.getenv("COINGECKO_API_KEY")
    
    # Pass the key to the bot's constructor
