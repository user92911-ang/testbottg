...
# --- Bot Logic (All Functions - Unchanged) ---
...
async def balance_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not addresses: await update.message.reply_text("❌ No valid Ethereum addresses found."); return
...
# --- NEW: LIFESPAN MANAGER TO START/STOP THE BOT ---
@asynccontextmanager
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    """Handles the startup and shutdown of the Telegram bot."""
    ...
    # Cleanly shut down the bot when the web server stops
    await application.updater.stop()
    await application.stop()
    await application.shutdown()
    logger.info("Telegram bot has been shut down.")

# --- WEB SERVER SETUP (Now with the lifespan manager) ---
...
# The if __name__ == "__main__" block is no longer needed, as Uvicorn is the entry point.